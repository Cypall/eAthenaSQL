// $Id: char-converter.c,v 1.2 2004/02/17 04:48:07 rovert Exp $
...
int storage_tosql(int account_id,struct storage *p){
	...
	for(i=0;i<MAX_STORAGE;i++) {
		...
		if( (p->storage[i].nameid) && (p->storage[i].amount) ){
			sprintf(tmp_sql,"INSERT INTO `storage` (`account_id`,`nameid`,`amount`,`equip`,`identify`,`refine`,`attribute`,`card0`,`card1`,`card2`,`card3`) VALUES ('%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d')",
...
int mmo_char_fromstr(char *str, struct mmo_charstatus *p){
  ...
  next++;
  for(i=0;str[next] && str[next]!='\t' && str[next]!='\n' && str[next]!='\r';i++){ //global_reg 실장 이전의 athena.txt 호환이기 때문에 일단'\n'체크
...
int mmo_char_tosql(int char_id, struct mmo_charstatus *p){
	...
	for(i=0;i<MAX_INVENTORY;i++){
		if(p->inventory[i].nameid){
			sprintf(tmp_sql,"INSERT INTO `inventory`(`id`,`char_id`, `nameid`, `amount`, `equip`, `identify`, `refine`, `attribute`, `card0`, `card1`, `card2`, `card3`)"
	...
	for(i=0;i<MAX_CART;i++){
		if(p->cart[i].nameid){
			sprintf(tmp_sql,"INSERT INTO `cart_inventory`(`id`,`char_id`, `nameid`, `amount`, `equip`, `identify`, `refine`, `attribute`, `card0`, `card1`, `card2`, `card3`)"
				" VALUES ('%d', '%d','%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d' )",
