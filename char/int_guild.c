...
int inter_guild_tosql(struct guild *g,int flag)
{
	...
	if (guild_exist >0){
		...
		sprintf(tmp_sql,"SELECT count(*) FROM `guild_member` WHERE `guild_id`='%d'",g->guild_id);
		if(mysql_query(&mysql_handle, tmp_sql) ) {
		...
		if (flag&8||guild_member==0){
			...
			sprintf(tmp_sql, "DELETE FROM `guild_alliance` WHERE `guild_id`='%d' OR `alliance_id`='%d';",g->guild_id,g->guild_id);
			if(mysql_query(&mysql_handle, tmp_sql) ) {
		...
		if (flag&2||guild_member==0){
			...
			sprintf(tmp_sql, "UPDATE `char` SET `guild_id`='0' WHERE `guild_id`='%d';",g->guild_id);
			if(mysql_query(&mysql_handle, tmp_sql) ) {
				printf("DB server Error (delete `guild_alliance`)- %s\n", mysql_error(&mysql_handle) );
			}
		}
		if (guild_member==0){
			...
			sprintf(tmp_sql, "DELETE FROM `guild_castle` WHERE `guild_id`='%d';",g->guild_id);
			if(mysql_query(&mysql_handle, tmp_sql) ) {
	...
	if (flag&2){
		...
		for(i=0;i<g->max_member;i++){
			if (g->member[i].account_id>0){
				...
				sprintf(tmp_sql, "UPDATE `char` SET `guild_id`='%d' WHERE `account_id`='%d' AND `char_id`='%d';",g->guild_id,m->account_id,m->char_id);
				if(mysql_query(&mysql_handle, tmp_sql) ) {
	...
	if (flag&32){
		...
		for(i=0;i<MAX_GUILDSKILL;i++){
			if (g->skill[i].id>0){
				sprintf(tmp_sql,"INSERT INTO `guild_skill` (`guild_id`,`id`,`lv`) VALUES ('%d','%d','%d');",
	...
	printf("Save guild %d successfully\n",g->guild_id);
	return 0;
}
...
int inter_guild_fromsql(int guild_id,struct guild *g)
{
	...
	sprintf(tmp_sql,"SELECT `guild_id`, `name`,`master`,`guild_lv`,`connect_member`,`max_member`,`average_lv`,`exp`,`next_exp`,`skill_point`,`castle_id`,`mes1`,`mes2`,`emblem_len`,`emblem_id`,`emblem_data` "
		"FROM `guild` WHERE `guild_id`='%d';",guild_id);
	//printf("  %s\n",tmp_sql);
	if(mysql_query(&mysql_handle, tmp_sql) ) {
	...
	sprintf(tmp_sql,"SELECT `guild_id`,`account_id`,`char_id`,`hair`,`hair_color`,`gender`,`class`,`lv`,`exp`,`exp_payper`,`online`,`position`,`rsv1`,`rsv2`,`name` "
		"FROM `guild_member` WHERE `guild_id`='%d'  ORDER BY `position`;", guild_id);
	//printf("  %s\n",tmp_sql);
	if(mysql_query(&mysql_handle, tmp_sql) ) {
	...
	sprintf(tmp_sql,"SELECT `guild_id`,`position`,`name`,`mode`,`exp_mode` FROM `guild_position` WHERE `guild_id`='%d';",guild_id);
	//printf("  %s\n",tmp_sql);
	if(mysql_query(&mysql_handle, tmp_sql) ) {
	...
	sprintf(tmp_sql,"SELECT `guild_id`,`opposition`,`alliance_id`,`name` FROM `guild_alliance` WHERE `guild_id`='%d';",guild_id);
	if(mysql_query(&mysql_handle, tmp_sql) ) {
	...
	sprintf(tmp_sql,"SELECT `guild_id`,`name`,`mes`,`acc`,`account_id`,`rsv1`,`rsv2`,`rsv3` FROM `guild_expulsion` WHERE `guild_id`='%d';",guild_id);
	if(mysql_query(&mysql_handle, tmp_sql) ) {
	...
	sprintf(tmp_sql,"SELECT `guild_id`,`id`,`lv` FROM `guild_skill` WHERE `guild_id`='%d' ORDER BY `id`;",guild_id);
	if(mysql_query(&mysql_handle, tmp_sql) ) {
...
int inter_guildcastle_tosql(struct guild_castle *gc)
{
	...
	sprintf(tmp_sql,"INSERT INTO `guild_castle` "
		"(`castle_id`, `guild_id`, `economy`, `defense`, `triggerE`, `triggerD`, `nextTime`, `payTime`, `createTime`, `visibleC`,"
	...
	sprintf(tmp_sql,"UPDATE `guild` SET `castle_id`='-1' WHERE `castle_id`='%d';",gc->castle_id);
	//printf(" %s\n",tmp_sql);
	if(mysql_query(&mysql_handle, tmp_sql) ) {
		printf("DB server Error - %s\n", mysql_error(&mysql_handle) );
		return 0;
	...
	sprintf(tmp_sql,"UPDATE `guild` SET `castle_id`='%d' WHERE `guild_id`='%d';",gc->castle_id,gc->guild_id);
	//printf(" %s\n",tmp_sql);
	if(mysql_query(&mysql_handle, tmp_sql) ) {
		printf("DB server Error - %s\n", mysql_error(&mysql_handle) );
		return 0;
...
int inter_guildcastle_fromsql(int castle_id,struct guild_castle *gc)
{
	...
	sprintf(tmp_sql,"SELECT `castle_id`, `guild_id`, `economy`, `defense`, `triggerE`, `triggerD`, `nextTime`, `payTime`, `createTime`, "
		...
		" FROM `guild_castle` WHERE `castle_id`='%d';",castle_id);
	if(mysql_query(&mysql_handle, tmp_sql) ) {
		printf("DB server Error - %s\n", mysql_error(&mysql_handle) );
		return 0;
	}
...
int inter_guild_sql_init()
{
	...
	guild_pt = malloc(sizeof(struct guild));
	guild_pt2=malloc(sizeof(struct guild));
	guildcastle_pt=malloc(sizeof(struct guild_castle));
	
	inter_guild_readdb(); // Read exp
	
	sprintf(tmp_sql,"UPDATE `guild_member` SET `online`='0';");	
	if(mysql_query(&mysql_handle, tmp_sql) ) {
		printf("DB server Error (update `char`)- %s\n", mysql_error(&mysql_handle) );
		exit(0);
	}
	
	sprintf (tmp_sql , "SELECT count(*) FROM `guild`");
	if(mysql_query(&mysql_handle, tmp_sql) ) {
		printf("DB server Error - %s\n", mysql_error(&mysql_handle) );
		exit(0);
	}
	...
	if (i > 0) {
		//set party_newid
		sprintf (tmp_sql , "SELECT max(`guild_id`) FROM `guild`");
		if(mysql_query(&mysql_handle, tmp_sql) ) {
			printf("DB server Error - %s\n", mysql_error(&mysql_handle) );
		}
...
int mapif_parse_BreakGuild(int fd,int guild_id)
{
	...
	printf("- Delete guild %d from guild\n",guild_id);
	sprintf(tmp_sql, "DELETE FROM `guild` WHERE `guild_id`='%d';",guild_id);
	if(mysql_query(&mysql_handle, tmp_sql) ) {
		printf("DB server Error (delete `guild`)- %s\n", mysql_error(&mysql_handle) );
	}
	printf("- Delete guild %d from guild_member\n",guild_id);
	sprintf(tmp_sql, "DELETE FROM `guild_member` WHERE `guild_id`='%d';",guild_id);
	if(mysql_query(&mysql_handle, tmp_sql) ) {
		printf("DB server Error (delete `guild_member`)- %s\n", mysql_error(&mysql_handle) );
	}
	printf("- Delete guild %d from guild_skill\n",guild_id);
	sprintf(tmp_sql, "DELETE FROM `guild_skill` WHERE `guild_id`='%d';",guild_id);
	if(mysql_query(&mysql_handle, tmp_sql) ) {
		printf("DB server Error (delete `guild_skill`)- %s\n", mysql_error(&mysql_handle) );
	}
	printf("- Delete guild %d from guild_position\n",guild_id);
	sprintf(tmp_sql, "DELETE FROM `guild_position` WHERE `guild_id`='%d';",guild_id);
	if(mysql_query(&mysql_handle, tmp_sql) ) {
		printf("DB server Error (delete `guild_position`)- %s\n", mysql_error(&mysql_handle) );
	}
	printf("- Delete guild %d from guild_expulsion\n",guild_id);
	sprintf(tmp_sql, "DELETE FROM `guild_expulsion` WHERE `guild_id`='%d';",guild_id);
	if(mysql_query(&mysql_handle, tmp_sql) ) {
		printf("DB server Error (delete `guild_expulsion`)- %s\n", mysql_error(&mysql_handle) );
	}
	printf("- Delete guild %d from guild_alliance\n",guild_id);
	sprintf(tmp_sql, "DELETE FROM `guild_alliance` WHERE `guild_id`='%d' OR `alliance_id`='%d';",guild_id,guild_id);
	if(mysql_query(&mysql_handle, tmp_sql) ) {
		printf("DB server Error (delete `guild_position`)- %s\n", mysql_error(&mysql_handle) );
	}
	
	printf("- Delete guild %d from guild_castle\n",guild_id);
	sprintf(tmp_sql, "DELETE FROM `guild_castle` WHERE `guild_id`='%d';",guild_id);
	if(mysql_query(&mysql_handle, tmp_sql) ) {
		printf("DB server Error (delete `guild_position`)- %s\n", mysql_error(&mysql_handle) );
	}
	
	printf("- Update guild %d of char\n",guild_id);
	sprintf(tmp_sql, "UPDATE `char` SET `guild_id`='0' WHERE `guild_id`='%d';",guild_id);
	if(mysql_query(&mysql_handle, tmp_sql) ) {
		printf("DB server Error (delete `guild_position`)- %s\n", mysql_error(&mysql_handle) );
	}
