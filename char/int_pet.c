...
int inter_pet_tosql(int pet_id, struct s_pet *p) {
	...
	if (mysql_num_rows(sql_res)>0) 
		...
		sprintf(tmp_sql,"INSERT INTO `pet` (`pet_id`, `class`,`name`,`account_id`,`char_id`,`level`,`egg_id`,`equip`,`intimate`,`hungry`,`rename_flag`,`incuvate`) VALUES ('%d', '%d', '%s', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d');",
			pet_id, p->class, t_name, p->account_id, p->char_id, p->level, p->egg_id,
...
int inter_pet_fromsql(int pet_id, struct s_pet *p){
	...
	sprintf(tmp_sql,"SELECT `pet_id`, `class`,`name`,`account_id`,`char_id`,`level`,`egg_id`,`equip`,`intimate`,`hungry`,`rename_flag`,`incuvate` FROM `pet` WHERE `pet_id`='%d'",pet_id);
	if(mysql_query(&mysql_handle, tmp_sql) ) {
...
int inter_pet_sql_init(){
	...
	pet_pt = malloc(sizeof(struct s_pet));
	
	sprintf (tmp_sql , "SELECT count(*) FROM `pet`");
	if(mysql_query(&mysql_handle, tmp_sql) ) {
			printf("DB server Error - %s\n", mysql_error(&mysql_handle) );
	}
	sql_res = mysql_store_result(&mysql_handle) ;
	...
	if (i > 0) {
		//set pet_newid
		sprintf (tmp_sql , "SELECT max(`pet_id`) FROM `pet`");
		if(mysql_query(&mysql_handle, tmp_sql) ) {
				printf("DB server Error - %s\n", mysql_error(&mysql_handle) );
		}
