...
int inter_party_tosql(int party_id,struct party *p)
{
	...
	if (party_exist >0){
		...
		if (party_member <= 0 && party_online_member == MAX_PARTY) {
			...
			for (i=0;i<MAX_PARTY;i++){
				...
				if (p->member[i].account_id>0){
					sprintf(tmp_sql,"UPDATE `char` SET `party_id`='%d', `online`='%d' WHERE `account_id`='%d' AND `name`='%s';",party_id, p->member[i].online, p->member[i].account_id,p->member[i].name);	
					//printf("%s",tmp_sql);
					if(mysql_query(&mysql_handle, tmp_sql) ) {
		...
		sprintf(tmp_sql,"UPDATE `char` SET `party_id`='%d', `online`='1'  WHERE `account_id`='%d' AND `name`='%s';",party_id,leader_id, p->member[i].name);	
		if(mysql_query(&mysql_handle, tmp_sql) ) {
			printf("DB server Error (inset/update `party`)- %s\n", mysql_error(&mysql_handle) );
		}
...
{
	int leader_id=0;
	printf("Request load party: %d.......\n",party_id);
	
	memset(p, 0, sizeof(struct party));
	
	sprintf(tmp_sql,"SELECT `party_id`, `name`,`exp`,`item`, `leader_id` FROM `party` WHERE `party_id`='%d'",party_id);
	if(mysql_query(&mysql_handle, tmp_sql) ) {
	...
	// Load members
	sprintf(tmp_sql,"SELECT `account_id`, `name`,`base_level`,`last_map`,`online` FROM `char` WHERE `party_id`='%d'",party_id);
	if(mysql_query(&mysql_handle, tmp_sql) ) {
		printf("DB server Error (select `party`)- %s\n", mysql_error(&mysql_handle) );
		return 0;
	}
...
int inter_party_sql_init(){
	...
	party_pt = malloc(sizeof(struct party));
	
	sprintf(tmp_sql,"UPDATE `char` SET `online`='0';");	
	if(mysql_query(&mysql_handle, tmp_sql) ) {
		printf("DB server Error (update `char`)- %s\n", mysql_error(&mysql_handle) );
	}
	
	sprintf (tmp_sql , "SELECT count(*) FROM `party`");
	if(mysql_query(&mysql_handle, tmp_sql) ) {
			printf("DB server Error - %s\n", mysql_error(&mysql_handle) );
	}
	sql_res = mysql_store_result(&mysql_handle) ;
	...
	if (i > 0) {
		//set party_newid
		sprintf (tmp_sql , "SELECT max(`party_id`) FROM `party`");
		if(mysql_query(&mysql_handle, tmp_sql) ) {
				printf("DB server Error - %s\n", mysql_error(&mysql_handle) );
		}
...
struct party* search_partyname(char *str)
{
	...
	sprintf(tmp_sql,"SELECT `party_id`, `name`,`exp`,`item`,`leader_id` FROM `party` WHERE `name`='%s'",str);	
	if(mysql_query(&mysql_handle, tmp_sql) ) {
			printf("DB server Error (select `party`)- %s\n", mysql_error(&mysql_handle) );
	}
	sql_res = mysql_store_result(&mysql_handle) ;
